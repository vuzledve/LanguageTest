...
namespace LanguageTest.Tests
{
    internal class Test2 : Base.BaseTest
    {
        ...
        {
            return "Если в базовом классе метод не является виртуальным (мы не можем его переопределить), но нас не устраивает его реализация для производного класса, то мы можем воспользоваться сокрытием new чтобы определить нужный нам функционал. " +
                   "Скрытие работает с переменными (статическими и нестатическими), константами, свойствами и методами. " +
                   "Если объявить в классе-наследнике метод с наименованием, которое уже есть в базовом классе, но не указать new, то такой код работает как будто мы указали new (только VS ругается предупреждениями).";
        }
